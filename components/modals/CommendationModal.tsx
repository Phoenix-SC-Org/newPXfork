...
const CommendationModal: React.FC<CommendationModalProps> = ({ isOpen, onClose, commendation }) => {
    ...
    const handleSubmit = useCallback(async (e: React.FormEvent) => {
        ...
        try {
            ...
            addToast(t("Save Failed"), <i className="fa-solid fa-xmark"></i>, "bg-red-500/10 text-red-400 border-red-500/50", { description: t("An error occurred while saving the commendation. Please try again.") });
            setIsLoading(false);
        }
    }, [name, description, icon, imageUrl, iconMode, isEditing, commendation, addCommendation, updateCommendation, onClose, addToast, t]);
    ...
    return (
        <WindowFrame
            isOpen={isOpen}
            onClose={onClose}
            title={isEditing ? t('Edit Commendation') : t('Create Commendation')}
            subtitle={t('Awards & Ribbons')}
        ...
        >
            <form onSubmit={handleSubmit} className="flex flex-col h-full">
                ...
                <div className="p-6 space-y-6 overflow-y-auto custom-scrollbar flex-1">
                    <div>
                        <label htmlFor="commendName" className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">{t('Commendation Name')}</label>
                        <input
                            type="text"
                            id="commendName"
                            value={name}
                            onChange={(e) => setName(e.target.value)}
                            placeholder={t('e.g., Medal of Valor')}
                    ...
                    <div>
                        <label htmlFor="commendDescription" className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">{t('Description (Optional)')}</label>
                        <textarea
                            id="commendDescription"
                            value={description}
                            onChange={(e) => setDescription(e.target.value)}
                            rows={3}
                            placeholder={t('e.g., Awarded for exceptional bravery in the face of overwhelming odds.')}
                ...
                <div className="flex justify-end items-center p-6 bg-slate-900/50 border-t border-white/5 rounded-b-2xl shrink-0 gap-3">
                    ...
                    <button
                        ...
                        disabled={isLoading}
                    >
                        {isLoading ? <i className="fa-solid fa-spinner animate-spin"></i> : (isEditing ? t('Save Changes') : t('Create Commendation'))}
                    </button>
                </div>
            </form>
        </WindowFrame>
