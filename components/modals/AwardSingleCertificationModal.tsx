...
const AwardSingleCertificationModal: React.FC<AwardSingleCertificationModalProps> = ({ isOpen, onClose, user }) => {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            addToast(t("Error"), <i className="fa-solid fa-xmark"></i>, "bg-red-500/10 text-red-400 border-red-500/50", { description: t("An error occurred while awarding certification. Please try again.") });
        } finally {
            setIsLoading(false);
        }
    };
    ...
    return (
        <WindowFrame
            isOpen={isOpen}
            onClose={onClose}
            title={t('Award Certification: {name}', { name: user.name })}
        ...
        >
            <div className="flex flex-col h-full">
                ...
                <div className="p-6 space-y-4">
                    <div>
                        <label htmlFor="certification" className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">{t('Select Certification')}</label>
                        <select
                            id="certification"
                            ...
                            disabled={isLoading}
                        >
                            <option value="">{t('- Choose a certification -')}</option>
                            {availableCerts.map(cert => (
                                <option key={cert.id} value={cert.id}>{cert.name}</option>
                            ))}
                        </select>
                        {availableCerts.length === 0 && (
                            <p className="text-xs text-slate-500 mt-2 italic bg-slate-900/50 p-2 rounded-sm border border-slate-800 text-center">{t('This member has already been awarded all available certifications.')}</p>
                        )}
                    </div>
                </div>
