...
const AddProspectModal: React.FC<AddProspectModalProps> = ({ isOpen, onClose }) => {
    ...
    const handleSubmit = useCallback(async (e: React.FormEvent) => {
        ...
        if (!rsiHandle.trim() || !applicantName.trim()) {
            addToast(t('Validation Error'), <i className="fa-solid fa-triangle-exclamation"></i>, "bg-amber-500/10 text-amber-400 border-amber-500/50", { description: t('RSI Handle and Name are required.') });
            return;
        }
        ...
        try {
            ...
            onClose();
            addToast(t('Case File Opened'), <i className="fa-solid fa-folder-plus"></i>, "bg-emerald-500/10 text-emerald-400 border-emerald-500/50", { description: t('New case file created successfully.') });
            refreshHR();
        } catch (err) {
            console.error("Failed to add prospect:", err);
            addToast(t('Error'), <i className="fa-solid fa-xmark"></i>, "bg-red-500/10 text-red-400 border-red-500/50", { description: t('An error occurred. Please check if this RSI handle is already in the system.') });
            setIsLoading(false);
        }
    }, [rsiHandle, applicantName, referral, notes, rpcAction, refreshHR, onClose, currentUser, addToast, t]);
    ...
    return (
        <WindowFrame
            isOpen={isOpen}
            onClose={onClose}
            title={t('Open New Case File')}
            subtitle={t('Recruitment & Vetting')}
            ...
            <form onSubmit={handleSubmit} className="flex flex-col h-full">
                <div className="p-6 space-y-6 overflow-y-auto custom-scrollbar flex-1">
                    <div className="bg-amber-900/10 border border-amber-500/20 p-4 rounded-xl text-xs text-amber-200">
                        ...
                        {t('Enter the candidate or subject details. If their RSI Handle matches a registered user, they will be automatically linked.')}
                    </div>
                    ...
                    <div>
                        ...
                        <div className="relative">
                            <input
                                ...
                                placeholder={t('e.g. StarCitizenPlayer')}
                                className={`${inputClass} ${linkedUser ? 'border-green-500 focus:border-green-500' : ''}`}
                                required
                                disabled={isLoading}
                            />
                            {linkedUser && (
                                <div className="absolute right-3 top-3 text-green-400" title={t('Linked to registered user')}>
                                    <i className="fa-solid fa-link"></i>
                                </div>
                            )}
                        </div>
                        {linkedUser && <p className="text-[10px] text-green-400 mt-1 font-bold">{t('Matched with user: {name}', { name: linkedUser.name })}</p>}
                    </div>

                    <div>
                        <label className={labelClass}>{t('Subject Name')} <span className="text-red-400">*</span></label>
                        <input
                            type="text"
                            value={applicantName}
                            onChange={(e) => setApplicantName(e.target.value)}
                            placeholder={t('Discord Display Name')}
                    ...
                    <div>
                        <label className={labelClass}>{t('Context / Referral Source')}</label>
                        <input
                            type="text"
                            value={referral}
                            onChange={(e) => setReferral(e.target.value)}
                            placeholder={t('e.g. Recruitment, Investigation, Internal Review')}
                    ...
                    <div>
                        <label className={labelClass}>{t('Initial Notes')}</label>
                        <textarea
                            value={notes}
                            onChange={(e) => setNotes(e.target.value)}
                            rows={3}
                            className={`${inputClass} resize-none`}
                            placeholder={t('Any initial observations...')}
                            disabled={isLoading}
                        />
                    </div>
                </div>
                ...
                <div className="p-4 border-t border-white/5 bg-slate-900/50 flex justify-end gap-3 rounded-b-xl">
                    ...
                    <button
                        ...
                        disabled={isLoading}
                    >
                        {isLoading ? <i className="fa-solid fa-spinner animate-spin"></i> : t('Open File')}
                    </button>
                </div>
            </form>
        </WindowFrame>
