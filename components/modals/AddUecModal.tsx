...
const AddUecModal: React.FC<AddUecModalProps> = ({ isOpen, onClose, operation }) => {
    ...
    const handleSubmit = useCallback(async (e: React.FormEvent) => {
        ...
        if (!reason.trim() || isNaN(numAmount) || numAmount <= 0) {
            addToast("Validation Error", <i className="fa-solid fa-triangle-exclamation"></i>, "bg-amber-500/10 text-amber-400 border-amber-500/50", { description: "Please enter a valid amount and reason." });
            return;
        }
        ...
        try {
            ...
            addToast("Error", <i className="fa-solid fa-xmark"></i>, "bg-red-500/10 text-red-400 border-red-500/50", { description: "An error occurred while adding aUEC. Please try again." });
            setIsLoading(false);
        }
    }, [amount, reason, operation, addOperationUec, onClose, addToast]);
        ...
        >
            <form onSubmit={handleSubmit} className="flex flex-col h-full">
                <div className="p-6 space-y-6">
                    <div>
                        <label className={labelClass}>aUEC Amount</label>
                    ...
                    <div>
                        <label className={labelClass}>Reason / Source</label>
                        <input
                            type="text"
                            value={reason}
                            onChange={(e) => setReason(e.target.value)}
                            placeholder="e.g., Contract Payout"
                ...
                <div className="p-4 border-t border-white/5 bg-slate-900/50 flex justify-end gap-3 rounded-b-xl">
                    ...
                    <button
                        ...
                        disabled={isLoading}
                    >
                        {isLoading ? <i className="fa-solid fa-spinner animate-spin"></i> : 'Add to Pool'}
                    </button>
                </div>
            </form>
        </WindowFrame>
