...
const OpAARTab: React.FC<OpAARTabProps> = ({ operation, canManage, isParticipant, onRefresh }) => {
    ...
    const handleReopenAAR = async () => {
        const ok = await confirm({
            title: t('Reopen AAR'),
            message: t('Reopen this AAR for editing? The summary and lessons learned are preserved; the submission will be cleared and the operation timeline will record this action.'),
        ...
        try {
            ...
            addToast(
                ...
                { description: err?.message || t('Could not reopen the AAR.') }
            );
        } finally {
            setReopening(false);
        }
    ...
    const handleGenerateDraft = async () => {
        ...
        try {
            ...
            addToast(
                ...
                { description: t('Review and edit the draft before submitting.') }
            );
        } catch (err: any) {
            ...
            const cleanMsg = rawMsg.startsWith('{') || rawMsg.includes('"error"')
                ? t('The AI service returned an unexpected error. Try again in a few minutes.')
                : rawMsg;
            ...
            if (cleanMsg.includes('AAR_COOLDOWN_ACTIVE') || err?.code === 'AAR_COOLDOWN_ACTIVE') {
                ...
                addToast(
                    t('AI draft on cooldown'),
                    <i className="fa-solid fa-hourglass-half" />,
                    'bg-amber-500/10 text-amber-300 border-amber-500/30',
                    { description: t('Drafts are limited to one every 3 hours per operation. Try again later.') }
                );
                return;
            }
            ...
            addToast(
                ...
                icon,
                'bg-red-500/10 text-red-400 border-red-500/50',
                { description: cleanMsg || t('Could not generate draft.') }
            );
        } finally {
        ...
        <div className="p-6 lg:p-8 space-y-6">
            <div className="flex flex-wrap items-center gap-3 p-4 bg-linear-to-r from-slate-800/60 to-slate-900/40 rounded-xl border border-slate-700/40">
                <p className="text-[10px] text-slate-500 uppercase font-black tracking-[0.15em] flex items-center gap-2">
                    <i className="fa-solid fa-file-lines text-purple-400/70"></i> {t('After Action Review')}
                    {isSubmitted && <span className="text-[8px] bg-green-500/10 text-green-400 border border-green-500/20 px-1.5 py-0.5 rounded-sm ml-1">{t('Submitted')}</span>}
                </p>
                ...
                {canContribute && !isSubmitted && (
                    <button onClick={() => setShowForm(!showForm)}
                        ...
                        <i className="fa-solid fa-plus"></i> {t('Add Entry')}
                    </button>
                )}
                {canManage && !isSubmitted && !editingSummary && (
                    <button onClick={() => { setSummaryText(operation.aarSummary || ''); setLessonsText(operation.aarLessonsLearned || ''); setEditingSummary(true); }}
                        ...
                        <i className="fa-solid fa-pen-to-square"></i> {t('Write Summary')}
                    </button>
                )}
            </div>
                ...
                <div className="bg-green-950/15 rounded-xl border border-green-500/15 overflow-hidden">
                    <div className="px-5 py-3 bg-green-950/20 border-b border-green-500/10">
                        <p className="text-[10px] text-green-400/80 uppercase font-black tracking-[0.15em] flex items-center gap-2">
                            <i className="fa-solid fa-check-circle"></i> {t('AAR Submitted')}
                        </p>
                    </div>
                    <div className="p-5 flex items-center justify-between gap-4 flex-wrap">
                        <p className="text-xs text-slate-400">
                            {t('Submitted {date}', { date: fmt(operation.aarSubmittedAt!) })}
                        </p>
                        {canReopen && (
                            <button
                                ...
                                title={t('Clear submission so the AAR can be edited again.')}
                            >
                                {reopening
                                    ? <><i className="fa-solid fa-spinner animate-spin"></i> {t('Reopening...')}</>
                                    : <><i className="fa-solid fa-rotate-left"></i> {t('Reopen AAR')}</>}
                            </button>
                        )}
                    </div>
                </div>
            ...
            {(editingSummary || operation.aarSummary || operation.aarLessonsLearned) && (
                <div className="bg-slate-900/60 rounded-xl border border-slate-700/30 overflow-hidden">
                    <div className="px-5 py-3 bg-slate-800/40 border-b border-slate-700/30">
                        <p className="text-[10px] text-slate-500 uppercase font-black tracking-[0.15em]">{t('AAR Summary')}</p>
                    </div>
                        ...
                        {editingSummary ? (
                                ...
                                <div className="flex items-center justify-between gap-3 p-3 rounded-lg border border-purple-500/20 bg-purple-500/5">
                                    <div className="flex-1 min-w-0">
                                        <p className="text-[10px] uppercase font-black tracking-widest text-purple-300 flex items-center gap-2">
                                            <i className="fa-solid fa-wand-magic-sparkles"></i> {t('AI Draft')}
                                        </p>
                                        <p className="text-[11px] text-slate-400 mt-1">
                                            {onCooldown
                                                ? t('Available again in {time}. Drafts are rate-limited to once every 3 hours per operation.', { time: formatRemaining(cooldownRemaining) })
                                                : t('Generate a draft summary and lessons-learned from the AAR entries. Review and edit before submitting.')}
                                        </p>
                                    </div>
                                    <button
                                        ...
                                        title={onCooldown ? t('Cooldown: {time} remaining', { time: formatRemaining(cooldownRemaining) }) : t('Generate AI draft')}
                                    >
                                        {generatingDraft
                                            ? <><i className="fa-solid fa-spinner animate-spin"></i> {t('Drafting...')}</>
                                            : onCooldown
                                                ? <><i className="fa-solid fa-hourglass-half"></i> {formatRemaining(cooldownRemaining)}</>
                                                : <><i className="fa-solid fa-wand-magic-sparkles"></i> {t('Generate Draft')}</>}
                                    </button>
                                </div>
                                <div>
                                    ...
                                    <textarea value={summaryText} onChange={e => setSummaryText(e.target.value)} rows={4} className={`${inputClass} w-full resize-none`}
                                        placeholder={t('Overall summary of the operation...')} />
                                </div>
                                <div>
                                    <label className={labelClass}>{t('Lessons Learned')}</label>
                                    <textarea value={lessonsText} onChange={e => setLessonsText(e.target.value)} rows={4} className={`${inputClass} w-full resize-none`}
                                        placeholder={t('Key takeaways and lessons...')} />
                                </div>
                                <div className="flex gap-3 justify-end">
                                    <button onClick={() => setEditingSummary(false)} className="text-xs text-slate-400 hover:text-white px-3 py-1.5 rounded-lg hover:bg-slate-800 transition-colors">{t('Cancel')}</button>
                                    <button onClick={handleSaveSummary} disabled={savingSummary}
                                        ...
                                        {savingSummary ? <i className="fa-solid fa-spinner animate-spin"></i> : t('Save Draft')}
                                    </button>
                                    <button onClick={handleSubmitAAR} disabled={savingSummary}
                                        ...
                                        {savingSummary ? <i className="fa-solid fa-spinner animate-spin"></i> : t('Submit AAR')}
                                    </button>
                                </div>
                            </div>
                        ) : (
                            <>
                                {operation.aarSummary ? (
                                    ...
                                    <p className="text-xs text-slate-600 italic">{t('No summary written yet.')}</p>
                                )}
                                {operation.aarLessonsLearned && (
                                    <div className="pt-3 border-t border-slate-700/30">
                                        <p className="text-[10px] text-slate-500 uppercase font-black tracking-widest mb-2">{t('Lessons Learned')}</p>
            ...
            {showForm && (
                <div className="bg-slate-900/60 rounded-xl border border-slate-700/30 overflow-hidden">
                    <div className="px-5 py-3 bg-slate-800/40 border-b border-slate-700/30">
                        <p className="text-[10px] text-slate-500 uppercase font-black tracking-[0.15em]">{t('New Entry')}</p>
                    </div>
                    <div className="p-5 space-y-4">
                        <div>
                            ...
                            <div className="flex gap-2 flex-wrap">
                                {Object.entries(categoryConfig).map(([key, cfg]) => (
                                    <button key={key} onClick={() => setCategory(key as AARCategory)}
                                        ...
                                        <i className={cfg.icon}></i> {t(cfg.label.split(' ')[0])}
                                    </button>
                                ))}
                            </div>
                        </div>
                        ...
                        <div className="flex justify-end gap-3">
                            <button onClick={() => setShowForm(false)} className="text-xs text-slate-400 hover:text-white px-3 py-1.5 rounded-lg hover:bg-slate-800 transition-colors">{t('Cancel')}</button>
                            <button onClick={handleAddEntry} disabled={saving || !content.trim()}
                                ...
                                {saving ? <i className="fa-solid fa-spinner animate-spin"></i> : t('Submit Entry')}
                            </button>
                        </div>
                    </div>
                </div>
                    ...
                    return (
                        <div key={key}>
                            <h4 className={`text-[10px] font-black uppercase tracking-widest mb-3 flex items-center gap-2 ${cfg.color}`}>
                                <i className={cfg.icon}></i> {t(cfg.label)}
            ...
            {entries.length === 0 && !canContribute && (
                <div className="flex flex-col items-center justify-center py-16 text-slate-600">
                    ...
                    <p className="text-sm font-medium opacity-50">{t('No AAR entries submitted yet.')}</p>
                </div>
            )}
        </div>
    );
