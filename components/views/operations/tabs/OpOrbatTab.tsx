        ...
        <div className="p-4 md:p-6 space-y-4 flex flex-col h-full">
            ...
            <div className="flex items-center justify-between shrink-0">
                ...
                <div className="flex items-center gap-2 bg-slate-800/60 border border-slate-700/50 rounded-lg px-3 py-2">
                    <span className="text-[9px] text-slate-500 font-black uppercase tracking-wider">{t('Ready')}</span>
                ...
                <>
                    ...
                    <div className="flex flex-wrap gap-3 items-center shrink-0">
                        {operation.maxParticipants && (
                            <div className="flex items-center gap-2 bg-slate-800/60 border border-slate-700/50 rounded-lg px-3 py-2">
                                <span className="text-[9px] text-slate-500 font-black uppercase tracking-wider">{t('Capacity')}</span>
                        ...
                        {canManage && onAddParticipant && (
                            <button onClick={onAddParticipant}
                                ...
                                <i className="fa-solid fa-user-plus"></i> {t('Add Participant')}
                            </button>
                        )}
                    </div>
                    ...
                    <div className="flex gap-3 items-center shrink-0">
                        <div className="relative flex-1">
                            ...
                            <input type="text" value={rosterSearch} onChange={e => setRosterSearch(e.target.value)} placeholder={t('Search participants...')}
                        ...
                        <select value={rosterSort} onChange={e => setRosterSort(e.target.value as any)}
                            ...
                            <option value="ready">{t('Sort: Ready')}</option>
                            <option value="name">{t('Sort: Name')}</option>
                            <option value="role">{t('Sort: Role')}</option>
                            <option value="joined">{t('Sort: Joined')}</option>
                        </select>
                    </div>
                    ...
                    <div className="flex-1 space-y-2 overflow-y-auto custom-scrollbar pr-1">
                        {sortedRosterParticipants.length > 0 ? (
                                    ...
                                    <div className="flex items-center space-x-3 min-w-0">
                                        ...
                                        <div className="min-w-0">
                                            <p className="font-bold text-sm text-white truncate flex items-center gap-1.5">
                                                ...
                                                {p.userId === operation.ownerId && <i className="fa-solid fa-crown text-[10px] text-amber-400" title={t('Lead')}></i>}
                                            </p>
                                            <div className="flex gap-2 items-center flex-wrap">
                                                <span className="text-[10px] text-slate-400 uppercase tracking-wider">{p.user?.rank?.name || t(p.user?.role || 'Member')}</span>
                                        ...
                                        {canManage && (
                                            <>
                                                <button onClick={() => onManageParticipant(p)}
                                                    className="text-slate-600 hover:text-purple-300 opacity-0 group-hover:opacity-100 transition-opacity p-2 hover:bg-purple-500/10 rounded-sm" title={t('Edit Participant')}>
                                                    <i className="fa-solid fa-pen-to-square"></i>
                                                </button>
                                                {p.userId !== operation.ownerId && (
                                                    <button onClick={async () => { const ok = await confirm({ title: t('Remove Participant'), message: t('Remove {name} from operation?', { name: p.user?.name }), confirmText: t('Remove'), variant: 'danger' }); if (ok) onRemoveParticipant(p.userId); }}
                                                        className="text-slate-600 hover:text-red-400 opacity-0 group-hover:opacity-100 transition-opacity p-2 hover:bg-red-500/10 rounded-sm" title={t('Remove Participant')}>
                            ...
                            <div className="flex flex-col items-center justify-center h-40 text-slate-600 opacity-50">
                                ...
                                <p className="text-xs italic">{t('No active participants.')}</p>
                            </div>
                        )}
                    </div>
                </>
                    ...
                    {canManage && (
                        <div className="flex items-center justify-end shrink-0">
                            <button onClick={() => openAddForm()} className="text-[10px] font-bold text-purple-300 hover:text-purple-200 uppercase">
                                <i className="fa-solid fa-plus mr-1"></i> {t('Add Node')}
                            </button>
                        </div>
                    )}
                        ...
                        <div className="bg-slate-800/40 border border-slate-700/50 rounded-lg p-4 space-y-3 animate-fade-in shrink-0">
                            <div className="grid grid-cols-3 gap-3">
                                <div>
                                    ...
                                    <input type="text" value={label} onChange={e => setLabel(e.target.value)} placeholder={t('e.g., Alpha Lead')} className={`${inputClass} w-full`} />
                                </div>
                                <div>
                                    <label className={labelClass}>{t('Node Type')}</label>
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                ...
                                <div>
                                    <label className={labelClass}>{t('Assign Participant')}</label>
                            ...
                            <div className="flex justify-end gap-2">
                                <button onClick={() => { setShowForm(false); setEditingNode(null); }} className="text-xs text-slate-400 hover:text-white px-3 py-1.5">{t('Cancel')}</button>
                                <button onClick={handleSaveNode} disabled={saving || !label.trim()} className="text-xs text-purple-300 bg-purple-500/10 border border-purple-500/30 hover:bg-purple-500/20 px-4 py-1.5 rounded-sm disabled:opacity-50">
                                    {saving ? <i className="fa-solid fa-spinner animate-spin"></i> : editingNode ? t('Update Node') : t('Add Node')}
                                </button>
                            </div>
                        </div>
                    )}
