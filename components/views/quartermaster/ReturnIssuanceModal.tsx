...
export default function ReturnIssuanceModal({ issuance, onClose, onSubmitted }: Props) {
    ...
    const submit = async () => {
        ...
        try {
            ...
            addToast('Issuance closed', <i className="fa-solid fa-check" />, 'bg-emerald-500/10 text-emerald-400 border-emerald-500/50');
            onSubmitted();
        } catch (err: any) {
            addToast('Return failed', <i className="fa-solid fa-xmark" />, 'bg-red-500/10 text-red-400 border-red-500/50', {
        ...
        >
            <div className="p-5 space-y-4">
                ...
                <label className="block">
                    <span className="text-[10px] font-mono uppercase tracking-widest text-slate-400">Returned quantity</span>
                    <input
                        type="number"
                        inputMode="numeric"
                        min={0}
                    ...
                    <span className="text-[10px] text-slate-500 mt-1 block">
                        {qty < issuance.quantity
                            ? `${issuance.quantity - qty} of ${issuance.quantity} not returned — track via notes.`
                ...
                <label className="block">
                    ...
                    <textarea
                        ...
                        rows={3}
                        maxLength={400}
                        placeholder="Anything relevant about condition, timing, damage?"
                ...
                <div className="flex justify-end gap-2 pt-2">
                    ...
                    >
                        {submitting ? 'Closing…' : 'Close Issuance'}
                    </button>
                </div>
            </div>
        </WindowFrame>
