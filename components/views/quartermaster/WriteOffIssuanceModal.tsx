...
export default function WriteOffIssuanceModal({ issuance, onClose, onSubmitted }: Props) {
    ...
    const submit = async () => {
        ...
        try {
            ...
            addToast(t('Issuance written off'), <i className="fa-solid fa-check" />, 'bg-emerald-500/10 text-emerald-400 border-emerald-500/50');
            onSubmitted();
        } catch (err: any) {
            addToast(t('Write-off failed'), <i className="fa-solid fa-xmark" />, 'bg-red-500/10 text-red-400 border-red-500/50', {
        ...
        >
            <div className="p-5 space-y-4">
                <p className="text-xs text-slate-400 bg-slate-900/60 p-3 rounded-sm border border-slate-800 leading-relaxed">
                    {t('The stock never comes back. No return movement is recorded — the issuance closes with the chosen outcome and the overall quantity on hand is permanently reduced by the issued amount.')}
                </p>
                ...
                <label className="block">
                    <span className="text-[10px] font-mono uppercase tracking-widest text-slate-400">{t('Notes')}</span>
                    <textarea
                        value={notes}
                        onChange={(e) => setNotes(e.target.value)}
                        rows={3}
                        maxLength={400}
                        placeholder={t('Circumstances, op reference, etc.')}
                ...
                <div className="flex justify-end gap-2 pt-2">
                    ...
                    >
                        {submitting ? t('Writing off…') : t('Confirm Write-Off')}
                    </button>
                </div>
            </div>
        </WindowFrame>
