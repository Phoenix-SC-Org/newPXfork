...
export default function CreateAccountModal({ onClose, onSubmitted }: Props) {
    ...
    const submit = async () => {
        ...
        try {
            ...
            addToast(t('Account created'), <i className="fa-solid fa-check" />, 'bg-emerald-500/10 text-emerald-400 border-emerald-500/50');
            onSubmitted();
        } catch (err: any) {
                ...
                <div>
                    ...
                    <div className="mt-1 grid grid-cols-2 gap-2">
                        {TYPE_OPTIONS.map((opt) => (
                            ...
                            >
                                <div className="font-bold uppercase tracking-widest">{t(opt.label)}</div>
                                <div className="text-[10px] text-slate-500 mt-0.5">{t(opt.hint)}</div>
                            </button>
                        ))}
                    </div>
                </div>
