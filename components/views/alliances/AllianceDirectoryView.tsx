...
const AllianceDirectoryView: React.FC = () => {
    ...
    return (
        <div className="h-full flex flex-col overflow-hidden animate-fade-in">
            <HeroShell
                chipLabel={t('MODULE · ALLIANCES')}
                chipIcon="fa-handshake"
                chipAccent="indigo"
                title={t('Alliance Directory')}
                subtitle={t('Organizations your org has a standing diplomatic relationship with.')}
                actions={<HeroActionButton onClick={load} accent="slate" icon="fa-rotate">{t('Refresh')}</HeroActionButton>}
                stats={<>
                    <HeroStat icon="fa-handshake" label={t('Total')} value={entries.length} accent="indigo" emphasize={entries.length > 0} />
                    <HeroStat icon="fa-circle-check" label={t('Active')} value={entries.filter((e) => e.status === 'Active').length} accent="emerald" emphasize={entries.some((e) => e.status === 'Active')} />
                    <HeroStat icon="fa-hourglass-half" label={t('Pending')} value={entries.filter((e) => e.status === 'Pending').length} accent="amber" emphasize={entries.some((e) => e.status === 'Pending')} />
                    <HeroStat icon="fa-bolt" label={t('Rivalries')} value={entries.filter((e) => e.type === 'Rivalry').length} accent="red" emphasize={entries.some((e) => e.type === 'Rivalry')} />
                </>}
            />

            <div className="flex-1 min-h-0 overflow-y-auto custom-scrollbar p-4 md:p-8 space-y-6">
            <div className="flex flex-wrap items-center gap-2">
                <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider mr-1">{t('Status')}</span>
                {(['All', 'Active', 'Pending'] as StatusFilter[]).map(s => (
                    <FilterChip key={s} active={statusFilter === s} onClick={() => setStatusFilter(s)}>{t(s)}</FilterChip>
                ))}
                <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider mx-1 ml-4">{t('Type')}</span>
                {(['All', 'Alliance', 'Neutral', 'Rivalry'] as TypeFilter[]).map(tf => (
                    <FilterChip key={tf} active={typeFilter === tf} onClick={() => setTypeFilter(tf)}>{t(tf)}</FilterChip>
                ))}
            </div>

            {loading ? (
                <p className="text-center text-slate-500 py-10">{t('Loading directory…')}</p>
            ) : filtered.length === 0 ? (
                <p className="text-center text-slate-500 py-10 italic">{t('No alliances match these filters.')}</p>
            ) : (
                            ...
                            <div key={entry.id} className={`bg-slate-900/40 rounded-xl border ${ts.ring} p-5 flex flex-col gap-3`}>
                                <div className="flex items-center gap-3">
                                    ...
                                    <div className="min-w-0">
                                        <p className="font-bold text-white truncate">{entry.peerOrgName || t('Unknown Org')}</p>
                                ...
                                <div className="mt-auto flex items-center gap-2 pt-1">
                                    <span className={`text-[10px] font-bold uppercase px-2 py-0.5 rounded-full border ${ts.chip}`}>
                                        <i className={`fa-solid ${ts.icon} mr-1`} />{t(entry.type)}
                                    </span>
                                ...
                                {entry.status === 'Active' && (
                                    <button onClick={() => openPeerDetail(entry)}
                                        ...
                                        <i className="fa-solid fa-users-viewfinder mr-1.5"></i>{t('View roster & fleet')}
                                    </button>
                                )}
                            </div>
                        );
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4" onClick={() => setOpenPeer(null)}>
                    <div className="bg-slate-900 border border-slate-700 rounded-xl w-full max-w-2xl max-h-[85vh] overflow-y-auto custom-scrollbar" onClick={e => e.stopPropagation()}>
                        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-700/50 sticky top-0 bg-slate-900">
                            <h3 className="text-lg font-bold text-white">{openPeer.peerOrgName || t('Ally')} <span className="text-slate-500 text-sm">· {t('shared intelligence')}</span></h3>
                            <button onClick={() => setOpenPeer(null)} className="text-slate-400 hover:text-white"><i className="fa-solid fa-xmark"></i></button>
                        </div>
                        <div className="p-6 space-y-6">
                            {detailLoading && <p className="text-center text-slate-500 py-6">{t('Fetching from {org}…', { org: openPeer.peerOrgName || t('the ally') })}</p>}

                            {!detailLoading && (
                                <div>
                                    <h4 className="text-sm font-bold text-white uppercase tracking-wider mb-3"><i className="fa-solid fa-users mr-2 text-slate-400"></i>{t('Member Roster')}</h4>
                                    {!roster ? (
                                        <p className="text-sm text-slate-500 italic">{t("This ally hasn't shared their roster.")}</p>
                                    ) : (
                                        <>
                                            <p className="text-xs text-slate-500 mb-3">{roster.memberCount === 1 ? t('{count} member', { count: roster.memberCount }) : t('{count} members', { count: roster.memberCount })}</p>
                                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 max-h-64 overflow-y-auto custom-scrollbar">
                                                {roster.members.map(m => (
                                                    <div key={m.id} className="flex items-center gap-2 p-2 rounded-lg bg-slate-800/40 border border-slate-700/40">
                                                        ...
                                                        {m.isDuty && <span className="ml-auto h-2 w-2 rounded-full bg-green-500" title={t('On duty')}></span>}
                                                    </div>
                                                ))}
                                            </div>
                                        </>
                            ...
                            {!detailLoading && (
                                <div>
                                    <h4 className="text-sm font-bold text-white uppercase tracking-wider mb-3"><i className="fa-solid fa-ship mr-2 text-slate-400"></i>{t('Fleet Summary')}</h4>
                                    {!fleet ? (
                                        <p className="text-sm text-slate-500 italic">{t("This ally hasn't shared their fleet.")}</p>
                                    ) : (
                                        <>
                                            <p className="text-xs text-slate-500 mb-3">{t('{ships} across {groups}', {
                                                ships: fleet.totalShips === 1 ? t('{count} ship', { count: fleet.totalShips }) : t('{count} ships', { count: fleet.totalShips }),
                                                groups: fleet.groupCount === 1 ? t('{count} group', { count: fleet.groupCount }) : t('{count} groups', { count: fleet.groupCount }),
                                            })}</p>
                                                ...
                                                <div className="space-y-1.5">
                                                    {fleet.groups.map(g => (
                                                        <div key={`${g.name}-${g.type}`} className="flex items-center justify-between text-xs p-2 rounded-lg bg-slate-800/30 border border-slate-700/30">
                                                            ...
                                                            <span className="text-slate-500">{g.totalShips === 1 ? t('{count} ship', { count: g.totalShips }) : t('{count} ships', { count: g.totalShips })}</span>
                                                        </div>
                                                    ))}
                                                </div>
                                            )}
