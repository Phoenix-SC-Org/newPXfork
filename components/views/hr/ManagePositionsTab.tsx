...
const ManagePositionsTab: React.FC = () => {
    ...
    const handleDelete = async (id: number) => {
        const confirmed = await confirm({ title: 'Delete Position', message: 'Are you sure you want to delete this position?', confirmText: 'Delete', variant: 'danger' });
        if (!confirmed) return;
        setDeletingId(id);
        try {
            ...
            console.error(e);
            addToast("Delete Failed", <i className="fa-solid fa-xmark"></i>, "bg-red-500/10 text-red-400 border-red-500/50", { description: "Failed to delete the position." });
        } finally {
            setDeletingId(null);
        }
    };
        ...
        <div className="space-y-6 animate-fade-in">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    ...
                    <p className="text-slate-400 text-sm mt-1">Define standardized job titles and functional roles.</p>
                </div>
            ...
            <div className="bg-slate-900/60 backdrop-blur-md rounded-xl border border-slate-700/50 overflow-hidden">
                {sortedAndFilteredItems.length > 0 ? (
                                ...
                                <tr key={pos.id} className="hover:bg-slate-800/40 transition-colors group">
                                    ...
                                    <td className="px-5 py-3 text-right">
                                        <div className="flex justify-end gap-1 opacity-60 group-hover:opacity-100 transition-opacity">
                                            <button onClick={() => openCreatePositionModal(pos)} className="p-2 text-emerald-300 hover:bg-emerald-500/10 rounded-lg transition-colors" title="Edit"><i className="fa-solid fa-pencil"></i></button>
                                            <button
                    ...
                    <EmptyState
                        icon="fa-briefcase"
                        accent="emerald"
                        heading="No job roles defined"
                        description={searchTerm ? 'Try a different search term.' : 'Create standardized roles to assign to members.'}
                    />
                )}
            </div>
        </div>
