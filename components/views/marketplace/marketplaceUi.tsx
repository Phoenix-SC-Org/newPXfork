...
export const Stars: React.FC<{ value: number; size?: string }> = ({ value, size = 'text-xs' }) => {
    const { t } = useI18n();
    return (
        <span className={`${size} text-amber-400`} aria-label={t('{value} stars', { value })}>
            {[1, 2, 3, 4, 5].map((i) => (
            ...
            <div className="flex-1 min-w-0 p-3">
                <div className="flex items-start justify-between gap-2">
                    ...
                    <span className={`shrink-0 text-[9px] font-black uppercase tracking-wider px-1.5 py-0.5 rounded-sm border ${meta.chip}`}>
                        <i className={`fa-solid ${meta.icon} mr-1`} aria-hidden />{t(meta.label)}
                    </span>
                </div>
                <div className="flex items-center gap-3 mt-1.5 text-[11px] text-slate-400">
                    ...
                    {listing.kind === 'item' && listing.quantity != null && (
                        <span><i className="fa-solid fa-layer-group mr-1 text-slate-500" aria-hidden />{t('{count} left', { count: Math.max(0, listing.quantity - listing.quantityClaimed) })}</span>
                    )}
